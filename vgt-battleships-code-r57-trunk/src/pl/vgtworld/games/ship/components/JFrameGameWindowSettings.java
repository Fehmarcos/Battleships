...
public class JFrameGameWindowSettings
    ...
    /**
     * The panel displayed after starting a new game. Performs the handling of
     * the deployment of ships by the player.
     */
    private JPanelMarkingShips oShipSelectionPanel;
    /**
     * The start panel displayed after starting the program with buttons to
     * start the game, change settings and end the game.
     */
    private JPanel oButtonsPanel;
    /**
     * Component displayed at the top of the window presenting messages about
     * board events of individual players.
     */
    private JComponentEvents oEventsComponent;
    /**
     * Component displayed at the bottom of the window that presents information
     ...
     *
     * Used to prevent a shot from being taken when it is the computer's turn or
     * the game is over victory of some player.
     */
    ...
    /**
     * Private class that handles the action of calling the game settings
     * window.
     */
     ...
     *
     * - giving the shot by the player by clicking on the computer board
     * performed by the mousePressed () method. <br />
     * - sending a shot by the computer to the player's board called by the
     * actionPerformed () method using a timer.
     */
    private class GameplayMouseListener
        ...
        public void mousePressed(MouseEvent event) {
            ...
            try {
                if (bPlayerTurn == true
                    ...
                    bPlayerTurn = false;
                    int iQuantitySunkenBeforeShot = oComputerShips.getNumberOfDestroyedShips();
                    ...
                    if (bHit == true && oComputerShips.getNumberOfShips() == oComputerShips.getNumberOfDestroyedShips()) {
                        ...
                        try {
                            String soundName = "vgt-battleships-code-r57-trunk\\src\\pl\\vgtworld\\games\\ship\\components\\arcade-game-explosion-2759.wav";
                        ...
                        oGameStatusComponent.updateData();
                        if (iQuantitySunkenBeforeShot < oComputerShips.getNumberOfDestroyedShips()) {
        ...
        public void actionPerformed(ActionEvent oEvent) {
            int iQuantitySunkenBeforeShot = oPlayerShips.getNumberOfDestroyedShips();
            ...
            if (bHit == true && oPlayerShips.getNumberOfShips() == oPlayerShips.getNumberOfDestroyedShips()) {
                ...
                oGameStatusComponent.updateData();
                if (iQuantitySunkenBeforeShot < oPlayerShips.getNumberOfDestroyedShips()) {
     ...
     * Constructor.
     *
     * @param oGameStatus Object that stores information about the current
     * status of the game.
     ...
     * @param bViewShips Variable whether or not missed ship positions are to be
     * displayed on board.
     */
    ...
    /**
     * The method adds the boards passed in the boards parameter to the
     * container. <br />
     *
     * The overloaded version, which additionally allows you to pass the
     * listener of clicks to the board.
     *
     * @param oBoard Board to be displayed in the board container.
     * @param bViewShips Variable whether or not missed ship positions are to be
     * displayed on board.
    ...
    /**
     * Method called by the ship placement panel after the player has confirmed
     * the correct placement of the player's ships.
     */
    ...
    /**
     * The method called by the Settings Window when there are changes to the
     * game settings (change of board size, number and / or size of ships,
     * difficulty level) <br />
     *
     * Corrects the required objects to match the new settings and if a game was
     * started, cancels it and starts a new one.
     */
    ...
    /**
     * The method creates a new board and a new container containing the
     * player's ships and returns the container object. <br />
     *
     * The size board, the number and the Size of the ships are determined by
     * the game settings given in the parameter.
     *
